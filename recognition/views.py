...
def create_dataset(username):
    ...
    while(True):
        ...
        cv2.imshow("Menambah foto",frame)
        #Before closing it we need to give a wait command, otherwise the open cv wont work
        # @params with the millisecond of delay 1
        cv2.waitKey(1)
        #To get out of the loop
        #Ambil jumlah sample wajah
        if(sampleNum>30):
            break
...
#used
def hours_vs_date_given_siswa(present_qs,time_qs,admin=True):
    ...
    if(admin):
        ...
        plt.savefig('./recognition/static/recognition/img/attendance_graphs/siswa_login/1.png')
        plt.close()
    return qs
    

#used
def hours_vs_siswa_given_date(present_qs,time_qs):
    register_matplotlib_converters()
    df_hours=[]
    df_break_hours=[]
    df_username=[]
    ...
    rcParams.update({'figure.autolayout': True})
    plt.tight_layout()
    plt.savefig('./recognition/static/recognition/img/attendance_graphs/hours_vs_siswa/1.png')
    plt.close()
    return qs


def total_number_siswas():
    qs=User.objects.all()
    return (len(qs) -1)
...
def this_week_sis_count_vs_date():
    ...
    while(cnt<5):
        ...
        if(str_dates.count(date))>0:
            ...
            sis_cnt_all.append(sis_count[idx])
        else:
            sis_cnt_all.append(0)

    df=pd.DataFrame()
    df["date"]=str_dates_all
    df["Jumlah siswa"]=sis_cnt_all 
    
    sns.lineplot(data=df,x='date',y='Jumlah siswa')
    plt.savefig('./recognition/static/recognition/img/attendance_graphs/this_week/1.png')
    plt.close()

#used
def last_week_sis_count_vs_date():
    today=datetime.date.today()
    some_day_last_week=today-datetime.timedelta(days=7)
    monday_of_last_week=some_day_last_week-  datetime.timedelta(days=(some_day_last_week.isocalendar()[2] - 1))
    monday_of_this_week = monday_of_last_week + datetime.timedelta(days=7)
    ...
    df=pd.DataFrame()
    df["date"]=str_dates_all
    df["hitung"]=sis_cnt_all
    
    sns.lineplot(data=df,x='date',y='hitung')
...
def dashboard(request):
    if(request.user.username=='admin'):
        ...
        return render(request,'recognition/siswa_dashboard.html')
...
def add_photos(request):
    ...
    if request.method=='POST':
        ...
        if username_present(username):
            ...
            messages.success(request, f'Dataset berhasil dibuat')
            return redirect('add-photos')
        else:
            messages.warning(request, f'Nama tidak ditemukan. Daftar siswa terlebih dahulu.')
...
def mark_your_attendance(request):
    ...
    while(True):
        ...
        #Showing the image in another window
        #Creates a window with window name "Face" and with the image img
        cv2.imshow("Absen - Masuk - Tekan q untuk exit",frame)
        #Before closing it we need to give a wait command, otherwise the open cv wont work
        # @params with the millisecond of delay 1
        #cv2.waitKey(1)
        #To get out of the loop
...
def mark_your_attendance_out(request):
    ...
    while(True):
        ...
        #Showing the image in another window
        #Creates a window with window name "Face" and with the image img
        cv2.imshow("Absen - Keluar - Tekan q untuk exit",frame)
        #Before closing it we need to give a wait command, otherwise the open cv wont work
        # @params with the millisecond of delay 1
        #cv2.waitKey(1)
        #To get out of the loop
...
def train(request):
    ...
    messages.success(request, f'Proses Training sudah selesai.')
...
def view_attendance_home(request):
    total_num_of_sis=total_number_siswas()
    sis_present_today=siswas_present_today()
    this_week_sis_count_vs_date()
    last_week_sis_count_vs_date()
    return render(request,"recognition/view_attendance_home.html", {'total_num_of_sis' : total_num_of_sis, 'sis_present_today': sis_present_today})
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if(len(time_qs)>0 or len(present_qs)>0):
                qs=hours_vs_siswa_given_date(present_qs,time_qs)
...
@login_required
def view_attendance_siswa(request):
    if request.user.username!='admin':
        return redirect('not-authorised')
    time_qs=None
    present_qs=None
        ...
        if form.is_valid():
            ...
            if username_present(username):
                ...
                if date_to < date_from:
                    ...
                    return redirect('view-attendance-siswa')
                else:
                    ...
                    if (len(time_qs)>0 or len(present_qs)>0):
                        qs=hours_vs_date_given_siswa(present_qs,time_qs,admin=True)
                        return render(request,'recognition/view_attendance_siswa.html', {'form' : form, 'qs' :qs})
                    else:
...
@login_required
def view_my_attendance_siswa_login(request):
    if request.user.username=='admin':
        return redirect('not-authorised')
    qs=None
    time_qs=None
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if date_to < date_from:
                    messages.warning(request, f'Invalid date selection.')
                    return redirect('view-my-attendance-siswa-login')
            else:           
                    ...
                    if (len(time_qs)>0 or len(present_qs)>0):
                        qs=hours_vs_date_given_siswa(present_qs,time_qs,admin=False)
                        return render(request,'recognition/view_my_attendance_siswa_login.html', {'form' : form, 'qs' :qs})
                    else:
...
def delete_photos(request):
    ...
    if request.method=='POST':
        ...
        username=data.get('username')
        if username_present(username):
            shutil.rmtree('face_recognition_data/training_dataset/{}/'.format(username))
            messages.success(request, f'Dataset berhasil dihapus.')
            return redirect('delete-photos')
        else:
            messages.warning(request, f'Nama tidak ditemukan. Daftar siswa terlebih dahulu.')
            return redirect('dashboard')
    else:
            form=usernameForm()
            return render(request,'recognition/delete_photos.html', {'form' : form})

def view_attendance_siswa_buka(request):
    time_qs=None
    present_qs=None
    qs=None
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if username_present(username):
                ...
                if date_to < date_from:
                    messages.warning(request, f'Tanggal yang dipilih salah.')
                    return redirect('view-attendance-siswa-buka')
                else:
                    time_qs=time_qs.filter(date__gte=date_from).filter(date__lte=date_to).order_by('-date')
                    present_qs=present_qs.filter(date__gte=date_from).filter(date__lte=date_to).order_by('-date')
                    ...
                    if (len(time_qs)>0 or len(present_qs)>0):
                        qs=hours_vs_date_given_siswa(present_qs,time_qs,admin=True)
                        return render(request,'recognition/view_attendance_siswa_buka.html', {'form' : form, 'qs' :qs})
                    else:
                        #print("inside qs is None")
                        messages.warning(request, f'Data pada periode ini tidak ditemukan.')
                        return redirect('view-attendance-siswa-buka')        
            else:
                print("invalid username")
                messages.warning(request, f'Nama tidak ditemukan.')
                return redirect('view-attendance-siswa-buka')
    else:
            form=UsernameAndDateForm()
            return render(request,'recognition/view_attendance_siswa_buka.html', {'form' : form, 'qs' :qs})
